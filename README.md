...
3. 选择一个或多个包含 `1.jpg`—`15000.jpg` 的 ZIP 并发布。若总量约 4GB，请拆成至少两个 ZIP，建议每个控制在 1.8–1.9GB。
...
- GitHub Release 的单个附件必须小于 2GiB，但同一 Release 的附件总大小不限；约 4GB 的素材请拆成两个或更多 ZIP。
- 建议每个 ZIP 控制在 1.8–1.9GB，避免刚好碰到 2GiB 边界；不同 ZIP 之间不得出现同一个数字文件名。
- 所有拆分 ZIP 必须放在同一个标签为 `photos-1-15000` 的 Release 中，系统会合并扫描和入库。
